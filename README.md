...
| 캘린더 피드 | [src/ics.ts](src/ics.ts) | 휴대폰 기본 캘린더용 iCalendar 구독 |
...
## 2. 로그인 — 링크 누르기 → 이름 고르기 → 숫자 4개

| 이름 | 누가 아는가 | 쓰는 곳 |
|---|---|---|
| **설치 코드** | 서버를 배포한 사람만 | 가족 공간을 **새로 만들 때**만. 모르는 사람이 내 서버에 공간을 만드는 것을 막는다 |
| **초대 링크** | 우리 가족 | 가족 공간에 **들어올 때**. 링크에 12자리 초대 코드가 담겨 있다 |
| **비밀번호** | 본인만 | 내 이름으로 로그인할 때. '나만 보기' 항목을 가리는 기준 |

### 처음 만들기
1. 배포된 주소를 **Safari/Chrome**으로 열기 → **가족 공간 새로 만들기**
2. 가족 이름 · 구성원 이름 · **설치 코드** 입력 (설치 코드는 배포할 때 한 번 표시된다)
3. 내 이름을 누르고 비밀번호 숫자 4개를 정한다
4. ⚙ 설정 → **가족 초대** → **초대 링크 보내기** → 카톡으로 전송

### 가족이 참여하기
링크를 누르면 이름 목록이 바로 뜬다 → 내 이름 누르기 → 처음이면 숫자 4개 정하기.
입력 칸이 없다. 한 번 로그인하면 **1년간** 다시 묻지 않는다.

링크를 누를 수 없는 경우(다른 기기 등)에는 첫 화면의 **초대 코드 직접 입력**에
링크 끝 12자리를 적는다. 하이픈·대소문자는 상관없다.

### 왜 가족 공용 PIN을 없앴나
예전에는 초대 코드(6자리) + 공용 PIN + 개인 PIN 세 개를 입력해야 했다.
초대 코드를 12자리 무작위 값(60비트)으로 늘려 링크에 담으면 추측이 사실상
불가능하므로(IP당 1분 10회 제한도 유지), 공용 PIN이 하던 "가족만 들어온다"는
역할을 링크가 대신한다.

링크가 모르는 사람에게 퍼졌다면 설정 → **새 초대 링크 만들기**. 이전 링크는 즉시
막히고, 이미 들어와 있는 가족의 로그인은 그대로 유지된다.
...
## 3-3. 기본 '미리 알림' 앱보다 편하게

| 기능 | 쓰는 법 | 기기 |
|---|---|---|
| **한 줄 빠른 추가** | 첫 화면 입력줄에 "내일 3시 치과" → 엔터. 날짜·시간·반복을 알아서 해석 | 전부 |
| **제목 자동 입력** | 일정 추가 화면 제목에 "매주 금요일 2시 반 미용실"을 치면 날짜·시간·반복 칸이 자동으로 채워지고 제목은 "미용실"로 저장 | 전부 |
| **휴대폰 기본 캘린더에 표시** | 설정 → 휴대폰 캘린더에 표시. 기본 캘린더 앱·위젯, "시리야 오늘 일정 뭐야?"로 확인 | 아이폰(바로), 안드로이드(구글 캘린더 웹에서 URL 추가) |
| **아이콘 숫자 배지** | 홈 화면 아이콘에 오늘 남은 일정 수 | 홈 화면에 추가한 아이폰(iOS 16.4+), 일부 안드로이드 런처 |
| **아이콘 길게 누르기** | 음성 추가 · 일정 추가 · 메모 추가 바로가기 | 안드로이드 |
| **공유하기로 추가** | 카톡 메시지 길게 누르기 → 공유 → 우리가족. 날짜가 있으면 일정, 없으면 메모로 제안 | 설치한 안드로이드 |

### 캘린더 구독 주소
`/api/calendar/<위젯토큰>.ics` ([src/ics.ts](src/ics.ts)). 위젯과 같은 토큰·같은 공개 범위를
쓴다(개인 주소 = 공유 + 내 개인 항목, 가족 주소 = 공유만). 읽기 전용이며 알림(VALARM)은
넣지 않는다 — 우리 푸시가 이미 알려주므로 넣으면 두 번 울린다.

새로 받아오는 주기는 기기가 정한다. 아이폰은 설정에서 15분까지 줄일 수 있고,
구글 캘린더는 몇 시간 간격이라 방금 넣은 일정은 앱 알림으로 먼저 보는 편이 낫다.
...
2. **초대 링크** — 가족 공간 진입. 12자리 무작위 코드(60비트), 재발급 가능.
3. **비밀번호** — 구성원 신원 확인. PBKDF2(10만 회, Workers 상한) 해싱.
4. **세션 토큰** — HMAC-SHA256 서명, 365일. 위조 시 401.

### 자동 차단
| 대상 | 제한 |
|---|---|
| 비밀번호 5회 연속 실패 | 해당 프로필 5분 잠금 |
| 가족 공간 생성 | IP당 1시간 5회 |
| 로그인 시도 | IP당 1분 10회 |
| 프로필 로그인 | IP당 1분 12회 |
| 위젯 조회 | IP당 1분 60회 |
